...
function DeskIllustration() {
  return (
    <svg viewBox="0 0 600 500" style={{ width: '100%', maxWidth: 460, height: 'auto' }}>
      <defs>
        <linearGradient id="lg"><stop offset="0%" stopColor="#0A2472" /><stop offset="100%" stopColor="#00072D" /></linearGradient>
      </defs>
      <circle cx="300" cy="250" r="200" fill="url(#lg)" opacity="0.15" />
    </svg>
  );
}
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(data?.error || data?.message || (res.status === 401 ? 'Invalid credentials' : `Error ${res.status}: ${res.statusText}`));
      }
    } catch (e: any) {
  ...
  return (
    <div className="login-wrapper">
      ...
      <div className="login-split" style={{ alignItems: 'center' }}>
        <div className="login-illustration"><div className="login-ill-content"><DeskIllustration /><div className="login-ill-text"><h2 className="login-ill-title">Tirbeo Admin</h2><p className="login-ill-desc">Manage your platform, monitor activity, and configure settings.</p></div></div></div>
        <div className="login-panel-wrap">
          <div className="login-panel" onMouseMove={handleTilt} onMouseLeave={resetTilt} style={{ transform: `rotateX(${tilt.rx}deg) rotateY(${tilt.ry}deg)` }}>
            ...
            <div className="login-panel-header">
              <div className="login-panel-logo"><div style={{ fontSize: 28 }}><ShieldIcon /></div></div>
              <h1 className="login-panel-title">{mode === 'login' ? 'Sign in' : 'Create account'}</h1>
              <p className="login-panel-sub">{mode === 'login' ? 'Welcome back, admin.' : 'Register for admin access.'}</p>
            </div>

            {error && <div className={`login-error-banner ${error.includes('created') ? 'login-success-banner' : ''}`}><AlertCircle />{error}</div>}
