...
const sections = [
  { key: "overview", name: "Overview" }, { key: "content", name: "Content" },
  { key: "communication", name: "Communication" }, { key: "users", name: "Users" },
  { key: "settings", name: "Settings" }, { key: "admin", name: "Admin" },
]
...
export default function Sidebar({ isOpen, onClose, userRole = "admin" }: { isOpen: boolean; onClose: () => void; userRole?: string }) {
  ...
  return (
    <>
      ...
      <aside className={`fixed top-0 left-0 z-50 h-full w-60 border-r border-border/40 bg-background transition-transform duration-300 ${isOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0`}>
        <div className="flex flex-col h-full">
          <div className="flex h-12 items-center justify-between px-4 border-b border-border/30">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold text-foreground">Tirbeo</span>
            </div>
              ...
              return (
                <div key={section.key} className="mb-4 last:mb-0">
                  <div className="px-3 mb-0.5">
                    <span className="text-[9px] text-muted-foreground/40 uppercase tracking-[0.15em] font-medium">{section.name}</span>
                  </div>
                      ...
                      <NavLink key={item.name} to={item.href} end={item.href === "/"} onClick={onClose}
                        className={({ isActive }) =>
                          `flex items-center gap-2.5 rounded-lg px-3 py-1.5 text-xs font-medium transition-all ${
                            isActive ? "bg-primary/10 text-primary" : "text-muted-foreground/70 hover:text-foreground hover:bg-muted/40"
                          }`
                        }>
